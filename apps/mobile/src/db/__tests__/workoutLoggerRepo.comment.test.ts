...
describe('updateWorkoutSessionExerciseComment', () => {
    ...
    it('adds or edits comment for in-progress sessions', () => {
        ...
        expect(exec).toHaveBeenCalledWith(expect.stringContaining('SET notes = ?'), ['warm up elbows', 'wse-1']);
        expect(enqueueOutboxOp).toHaveBeenCalledWith(expect.objectContaining({
    ...
    it('enforces a 200-character max length', () => {
        ...
        expect(exec).toHaveBeenCalledWith(expect.stringContaining('SET notes = ?'), ['a'.repeat(200), 'wse-1']);
    });
});