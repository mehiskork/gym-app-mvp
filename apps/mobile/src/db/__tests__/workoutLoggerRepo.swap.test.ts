...
describe('swapWorkoutSessionExercise', () => {
  ...
  it('inserts replacement below when completed sets exist', () => {
    ...
    expect(exec).toHaveBeenCalledWith(expect.stringContaining('SET position = position + ?'), [
      1000000,
      'ws-1',
      2,
    ]);
    expect(exec).toHaveBeenCalledWith(
      ...
      ['wse-inserted', 'ws-1', 'ex-2', 'Incline Bench Press', 'strength', null, 3],
    );
    expect(exec).toHaveBeenCalledWith(
  ...
  it('inserts replacement below when completed sets exist and current is last', () => {
    ...
    expect(exec).toHaveBeenCalledWith(
      expect.stringContaining('INSERT INTO workout_session_exercise'),
      ['wse-inserted', 'ws-1', 'ex-9', 'Cable Row', 'strength', null, 6],
    );
    expect(exec).toHaveBeenCalledWith(expect.stringContaining('SET position = position - ?'), [
      999999,
      'ws-1',
