...
class SyncProtocolInvariantsIT {
    ...
    void syncUsesLimitPlusOneFetchWithoutAdvancingCursorPastReturnedPage() throws Exception {
        ...
        long firstPageLastReturnedChangeId = firstResponse.path("deltas").get(deltaLimit - 1).path("changeId").asLong();
    ...
    void syncTreatsMixedPayloadReplayOfSameOpIdAsIdempotentNoop() throws Exception {
        ...
        assertThat(firstResponse.path("acks").get(0).path("opId").asText()).isEqualTo("op-1");
        assertThat(firstResponse.path("acks").get(0).path("status").asText()).isEqualTo("applied");
        ...
        assertThat(replayResponse.path("acks").get(0).path("opId").asText()).isEqualTo("op-1");
        assertThat(replayResponse.path("acks").get(0).path("status").asText()).isEqualTo("noop");
        assertThat(replayResponse.path("acks").get(0).path("reason").asText()).isEqualTo("duplicate op");
    ...
    private RegisteredDevice registerDevice() throws Exception {
        ...
        return new RegisteredDevice(
                response.path("deviceToken").asText(),
                response.path("guestUserId").asText());
    }
